...
#CELLBENDER_OUT = '/scratch/scjp_root/scjp0/porchard/pfizer-mouse-multiome/work/cellbender-all/results/6688-NM-3-mm10_chimera.cellbender_FPR_0.1.h5'
#PREFIX = 'test.'

parser = argparse.ArgumentParser()
parser.add_argument('cellbender_h5')
parser.add_argument('prefix')
parser.add_argument('--min-cell-probability', type=float, default=0.0, help='Minimum probability of being a cell in order to be included in the output')
#parser.add_argument('--keep-barcodes', default=None, help='Keep barcodes listed in this file')
args = parser.parse_args()
#CELLBENDER_OUT, PREFIX = sys.argv[1:]
...
adata = anndata_from_h5(args.cellbender_h5, analyzed_barcodes_only=True)
adata.var['feature_name'] = adata.var.gene_id + '\t' + adata.var.index + '\tGene Expression'

# keep_barcodes = adata.obs.index.to_list() if not args.keep_barcodes else pd.read_csv(args.keep_barcodes, header=None)[0].to_list()

MATRIX, FEATURES, BARCODES = [args.prefix + i for i in ['matrix.mtx', 'features.tsv', 'barcodes.tsv']]
barcodes = adata.obs[adata.obs.cell_probability>=args.min_cell_probability].index.to_list()
features = adata.var.feature_name.to_list()

io.mmwrite(MATRIX, adata[adata.obs.cell_probability>=args.min_cell_probability].X.transpose(), comment='', field='integer', symmetry='general')
